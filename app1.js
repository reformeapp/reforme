...
async function addFoodLog(name, cal, protein, carbs, fat, mealType) {
  ...
  if (currentProfile?.trainer_id) {
    sb.from('notifications').insert({
      ...
      title: (currentProfile.full_name?.split(' ')[0] || 'Member') + ' logged a meal',
...
async function toggleClientPlan(el, table, id) {
  ...
  if (table === 'workout_plans' && newStatus && currentProfile?.trainer_id) {
    ...
    if (workoutItem) {
      sb.from('notifications').insert({
        user_id: currentProfile.trainer_id,
        title: (currentProfile.full_name?.split(' ')[0] || 'Member') + ' completed an exercise',
        body: workoutItem.exercise_name,
        type: 'workout_log'
      }).then(() => {}).catch(() => {});
    }
  }
...
function updateTrainerProfile() {
  ...
  const hour = new Date().getHours();
  const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';
  const greetEl = document.getElementById('trainerGreetingSub');
...
async function loadTrainerClients() {
  ...
  clientList.forEach((client, i) => {
    ...
    const calGoal = client.calorie_goal || 1800;
    const calPct = loggedToday ? Math.min(100, Math.round((totalCal/calGoal)*100)) : 0;
    const isLow = loggedToday && calPct < 40;
    ...
    let cardBg, cardBorder, tagHtml, subText, barColor;
    if (!loggedToday) {
      cardBg = 'rgba(181,64,64,0.06)';
      cardBorder = 'rgba(181,64,64,0.2)';
      tagHtml = '<span style="font-size:10px;color:#B54040;background:rgba(181,64,64,0.15);padding:3px 8px;border-radius:20px;flex-shrink:0;">No log</span>';
      subText = `<div style="font-size:11px;color:rgba(181,64,64,0.7);margin-top:2px;">Hasn't logged today</div>`;
      barColor = '#B54040';
      attention++;
    } else if (isLow) {
      cardBg = 'rgba(193,125,60,0.06)';
      cardBorder = 'rgba(193,125,60,0.2)';
      tagHtml = '<span style="font-size:10px;color:#C17D3C;background:rgba(193,125,60,0.15);padding:3px 8px;border-radius:20px;flex-shrink:0;">Low</span>';
      subText = `<div style="font-size:11px;color:rgba(193,125,60,0.7);margin-top:2px;">${totalCal.toLocaleString()} kcal · below target</div>`;
      barColor = '#C17D3C';
      attention++;
    } else {
      cardBg = 'rgba(45,106,79,0.06)';
      cardBorder = 'rgba(45,106,79,0.2)';
      tagHtml = '<span style="font-size:10px;color:#2D6A4F;background:rgba(45,106,79,0.15);padding:3px 8px;border-radius:20px;flex-shrink:0;">On track</span>';
      subText = `<div style="font-size:11px;color:rgba(45,106,79,0.8);margin-top:2px;">${totalCal.toLocaleString()} kcal today</div>`;
      barColor = '#2D6A4F';
      onTrack++;
    }

    html += `<div onclick="openClientDetail('${client.id}','${safeName}','${color}')" style="background:${cardBg};border:0.5px solid ${cardBorder};border-radius:16px;padding:14px;margin-bottom:8px;cursor:pointer;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
        <div style="width:40px;height:40px;border-radius:50%;background:${color};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;color:#fff;flex-shrink:0;">${initial}</div>
        <div style="flex:1;">
          <div style="font-size:13px;font-weight:500;color:#fff;">${client.full_name||'Unknown'}</div>
          ${subText}
        </div>
        ${tagHtml}
      </div>
      <div style="height:1.5px;background:rgba(255,255,255,0.06);border-radius:2px;overflow:hidden;">
        <div style="width:${calPct}%;height:100%;background:${barColor};border-radius:2px;transition:width 0.5s;"></div>
      </div>
    </div>`;
  });
  container.innerHTML = html;
